...
@section('main')
    <div class="main-content group-data-[sidebar-size=lg]:xl:ml-[calc(theme('spacing.app-menu')_+_16px)] group-data-[sidebar-size=sm]:xl:ml-[calc(theme('spacing.app-menu-sm')_+_16px)] group-data-[theme-width=box]:xl:px-0 px-3 my-2 xl:px-4 ac-transition">
        <form action="{{route('updatecolor',$listcolor->id)}}" method="POST" class="container" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="card">
                <h2 class="card-title">thêm kích cỡ</h2>
            </div>
            <div class=" container">
                <div class="col-span-full lg:col-span-6 ">
                    <div class="card p-0">
                        <div class="flex-center-between p-6 pb-4 border-b border-gray-200 dark:border-dark-border">
                            <h3 class="text-lg card-title leading-none">thêm kích cỡ</h3>
                        </div>
                        <div class="p-6 space-y-4 ">
                            <div>
                                <div>
                                    <label for="name" class="form-label">Tên</label>
                                    <input type="text" id="name" value="{{$listcolor->name}}" name="name" class="form-input" autocomplete="off" required >
                                </div>
                            </div>
                        </div>
