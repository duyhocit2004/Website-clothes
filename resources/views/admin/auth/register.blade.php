    ...
    <div class="grid grid-cols-12 gap-y-7 sm:gap-7 card px-4 sm:px-10 2xl:px-[70px] py-15 lg:items-center lg:min-h-[calc(100vh_-_32px)]">
        ...
        <div class="col-span-full lg:col-span-6 w-full lg:max-w-[600px]">
            <div class="border border-form dark:border-dark-border p-5 md:p-10 rounded-20 md:rounded-30 dk-theme-card-square">
                ...
                <form action="{{route('register')}}" method="POST" class="leading-none mt-8">
                    @csrf
                    <div class="mb-2.5">
                        <label for="name" class="form-label">Name</label>
                        <input type="text" id="name" name="name" placeholder="Debra Holt" value="{{old('name')}}"  required class="form-input px-4 py-3.5 rounded-lg">
                        @error('name')
                    ...
                    <div class="mt-5">
                        ...
                        <input type="email" id="email" placeholder="debra.holt@example.com" name="email" value="{{old('email')}}" required class="form-input px-4 py-3.5 rounded-lg">
                        @error('email')
                        <p class="text-danger"> {{$message}}</p>
                        @enderror
                    </div>
                    <div class="mt-5">
                        ...
                        <div class="relative">
                            <input type="password" id="password" placeholder="Password" name="password" value="{{old('password')}}" required class="form-input px-4 py-3.5 rounded-lg">
                ...
                <div class="text-gray-900 dark:text-dark-text font-medium leading-none mt-5">
                    Have an account?
                    <a href="{{route('fromlogin')}}" class="text-primary-500 font-semibold">Sign In</a>
                </div>
            </div>
        </div>
        <!-- End Form Area -->
